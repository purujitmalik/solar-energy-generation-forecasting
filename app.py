...
def to_display_values(values: pd.Series | np.ndarray, target: str) -> pd.Series:
    series = pd.to_numeric(pd.Series(values), errors="coerce")
    divisor = TARGET_DIVISORS.get(target, 1.0)
    return series / divisor


def normalize_irradiance_kw_per_m2(values: pd.Series | np.ndarray) -> pd.Series:
    series = pd.to_numeric(pd.Series(values), errors="coerce").clip(lower=0)
    # Open-Meteo shortwave radiation arrives in W/m^2, while the bundled dataset is near kW/m^2.
    if series.dropna().quantile(0.99) > 20:
        return series / 1000.0
    return series


def estimate_weather_based_power(
    frame: pd.DataFrame,
    user_panel_count: int,
    panel_wattage: float,
    target: str,
) -> pd.Series:
    capacity_kw = max(user_panel_count, 1) * max(panel_wattage, 1.0) / 1000.0
    irr_kw_per_m2 = normalize_irradiance_kw_per_m2(frame["IRRADIATION"])
    module_temp = pd.to_numeric(
        frame["MODULE_TEMPERATURE"].fillna(frame["AMBIENT_TEMPERATURE"]).fillna(25.0),
        errors="coerce",
    )
    temp_factor = (1.0 - 0.004 * np.clip(module_temp - 25.0, 0, None)).clip(0.75, 1.02)
    performance_ratio = 0.90 if target == "AC_POWER" else 0.96
    cap_limit = capacity_kw * (0.92 if target == "AC_POWER" else 1.00)

    predicted_kw = capacity_kw * irr_kw_per_m2 * temp_factor * performance_ratio
    predicted_kw = predicted_kw.clip(lower=0, upper=cap_limit)
    return predicted_kw * TARGET_DIVISORS.get(target, 1.0)
...
) -> tuple[float, int, object, str, bool, bool, int]:
...
def render_dashboard_tabs(
    ...
    with tabs[0]:
        ...
        fig_hist.update_traces(y=to_display_values(merged[target], target))
    ...
    with tabs[1]:
        ...
        eval_fig.add_trace(
            go.Scatter(
                x=test_df["DATE_TIME"],
                y=to_display_values(test_df[target], target),
                mode="lines",
                name="Actual",
        ...
        eval_fig.add_trace(
            go.Scatter(
                x=test_df["DATE_TIME"],
                y=to_display_values(preds, target),
                mode="lines",
                name="Predicted",
    ...
    with tabs[2]:
        ...
        forecast_fig.add_trace(
            go.Scatter(
                x=merged["DATE_TIME"],
                y=to_display_values(merged[target] * panel_scale, target),
                mode="lines",
                name="History (scaled)",
                line=dict(color="#1a7bb9"),
            )
        ...
        forecast_fig.add_trace(
            go.Scatter(
                ...
                y=to_display_values(forecast_df["Forecast"], target),
                mode="lines",
    ...
    with tabs[3]:
        ...
        top_cols[0].metric(f"Predicted total ({total_unit})", f"{predicted_total:,.2f}")
        ...
        if not day_pred_df.empty:
            day_fig.add_trace(
                go.Scatter(
                    ...
                    y=to_display_values(day_pred_df["Prediction"], target),
                    mode="lines",
                    name="Predicted",
                    line=dict(color="#f28f3b"),
                )
        ...
        if actual_mask.any():
            ...
            day_fig.add_trace(
                go.Scatter(
                    ...
                    y=to_display_values(actual_day[target] * panel_scale, target),
                    mode="lines",
                    name="Actual (scaled)",
                    line=dict(color="#1a7bb9"),
                )
            )
        apply_chart_style(day_fig, "Daily Prediction Profile", profile_unit)
        st.plotly_chart(day_fig, use_container_width=True)


def calculate_daily_total(
    target: str, predictions: pd.Series | np.ndarray, freq: pd.Timedelta
) -> tuple[float, str]:
    values = pd.to_numeric(pd.Series(predictions), errors="coerce")
    if target in {"AC_POWER", "DC_POWER"}:
        hours_per_step = freq.total_seconds() / 3600.0
        return float(np.nansum(values.values) * hours_per_step / 1000.0), "kWh"
    return float(np.nanmax(values.values) / 1000.0), "kWh"
...
def main():
    ...
    if target in {"AC_POWER", "DC_POWER"} and use_weather:
        try:
            day_pred = estimate_weather_based_power(day_frame, user_panel_count, panel_wattage, target)
        except Exception:
            day_pred = model.predict(day_frame[feature_cols]) * panel_scale
    day_pred_df = pd.DataFrame(
    ...
    predicted_total, total_unit = calculate_daily_total(target, day_pred_df["Prediction"], freq_delta)
    ...
    profile_unit = TARGET_UNITS.get(target, "units")
